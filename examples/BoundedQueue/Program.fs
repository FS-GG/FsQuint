...
let hash (s: string) = SHA256.HashData(Encoding.UTF8.GetBytes(s)) |> Convert.ToHexStringLower
let state bindings =
...
let raw = File.ReadAllBytes(Path.Combine(AppContext.BaseDirectory, "queue.itf.json"))
let document = Itf.read Itf.defaultLimits raw |> unwrap
let states = document.States |> List.map (List.map (fun (k,v) -> k,legacy v) >> state)
...
let source = { Path = "scenario.qnt"; Line = 4; Column = 3 }
let environment = {
    Seed = "42"; Bounds = ["capacity",2L; "steps",4L]
...
let draft = {
    SchemaVersion = 1; TraceIdentity = ""; Environment = environment; Initial = states.Head
    Steps = List.zip actions states.Tail |> List.mapi (fun i (a,s) -> { Index=i+1; Action=a; Source=source; Expected=s })
}
let trace = { draft with TraceIdentity = QuintReplay.traceFingerprint draft |> unwrap }

type QueueRuntime = { Items: Collections.Generic.List<int>; Inserted: Collections.Generic.List<int>; Removed: Collections.Generic.List<int> }
let driver broken mutate = {
    Initialize = fun _ _ -> Task.FromResult(Ok { Items = new Collections.Generic.List<int>(); Inserted = new Collections.Generic.List<int>(); Removed = new Collections.Generic.List<int>() })
    Apply = fun step runtime _ -> task {
        match step.Action with
        | "enqueue:1" | "enqueue:2" ->
            if runtime.Items.Count = 2 then return Error "Queue full"
            else
                ...
                return Ok ()
        | "dequeue" ->
            if runtime.Items.Count = 0 then return Error "Queue empty"
            else
    ...
    Observe = fun runtime _ ->
        let values xs = xs |> Seq.map (string >> Integer) |> Seq.toList |> Sequence
        let items = if mutate then Sequence [] else values runtime.Items
        state ["state", Record ["items", items; "inserted", values runtime.Inserted; "removed", values runtime.Removed]] |> Ok |> Task.FromResult
    Cleanup = fun _ _ -> Task.FromResult(Ok ())
}
let run broken mutate = Replay.run (TimeSpan.FromSeconds(5.0)) CancellationToken.None (driver broken mutate) trace |> fun t -> t.GetAwaiter().GetResult()
let good = run false false
if good.Outcome <> ReplayOutcome.Equivalent || good.CleanupFailure.IsSome then failwithf "Positive control failed: %A" good
