...
type ReplayOutcome =
    | Equivalent
    | Diverged of step: int * action: string option * source: QuintReplaySourceBinding option * path: string * expected: QuintReplayState * actual: QuintReplayState
    | InvalidTrace of QuintReplayDiagnostic list
    | DriverFailure of phase: string * step: int * message: string
    | Cancelled
    | TimedOut

type ReplayReport = { Outcome: ReplayOutcome; CleanupFailure: string option; AppliedSteps: int }

[<RequireQualifiedAccess>]
module Replay =
    /// Deadline is cooperative: use process isolation for untrusted/non-cooperative drivers.
    let run (timeout: TimeSpan) (cancellation: CancellationToken) (driver: ReplayDriver<'runtime>) (trace: QuintReplayTrace) = task {
        if timeout <= TimeSpan.Zero || timeout.TotalMilliseconds > float UInt32.MaxValue - 1.0 then
        ...
        let fail message = outcome <- ReplayOutcome.DriverFailure(phase, index, message)
        let isEquivalent () = outcome = ReplayOutcome.Equivalent
        let observe expected action source = task {
            phase <- "observe"
            ...
            | Ok actual ->
                ...
                | Ok a, Ok b when a = b -> ()
                | Ok _, Ok _ ->
                    let names = (expected.Bindings @ actual.Bindings) |> List.map fst |> List.distinct |> List.sort
                    let differing = names |> List.tryFind (fun name ->
                        let encoded bindings = bindings |> List.tryFind (fst >> (=) name) |> Option.map (snd >> QuintReplay.encodeValue)
                        encoded expected.Bindings <> encoded actual.Bindings)
                    let path = differing |> Option.map (fun n -> "$/bindings/" + n.Replace("~", "~0").Replace("/", "~1")) |> Option.defaultValue "$"
        ...
        with
        | :? OperationCanceledException when cancellation.IsCancellationRequested -> outcome <- ReplayOutcome.Cancelled
        ...
        | Some value ->
            ...
            with ex -> cleanupFailure <- Some ex.Message
        return { Outcome = outcome; CleanupFailure = cleanupFailure; AppliedSteps = applied }
    }
